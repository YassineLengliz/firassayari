...
export class RuleBasedMedicalNlp implements MedicalNlpProvider {
  async structure(text: string): Promise<StructuredMedicalNote> {
    ...
    return {
      reason: /douleur|rage de dent|sensible|sensibilite|sensibilité/i.test(clean) ? "Douleur dentaire" : "Motif dentaire a confirmer",
      symptoms: [clean],
      diagnosticHypothesis: "Suggestion non automatique a valider par le dentiste",
      treatmentPlan: "Examen bucco-dentaire et conduite a tenir documentee par Dr Firas Sayari.",
      prescription: [],
      priceCents: 6000
    };
  }
